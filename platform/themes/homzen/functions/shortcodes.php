...
Event::listen(RouteMatched::class, function () {
    ...
    Shortcode::register(
        ...
        function (ShortcodeCompiler $shortcode) {
            ...
            return Theme::partial('shortcodes.services.index', compact('shortcode', 'services', 'counters'));
        }
    );
    ...
    Shortcode::setAdminConfig('services', function (array $attributes) {
        return ShortcodeForm::createFromArray($attributes)
            ...
            ->add(
                ...
                TextareaFieldOption::make()
                    ->label(__('Checklist'))
                    ->helperText(__('Enter checklist here, saperated by commas (,)'))
    ...
    Shortcode::setAdminConfig('hero-banner', function (array $attributes) {
        $form = ShortcodeForm::createFromArray($attributes)
            ...
            ->when(is_plugin_active('real-estate'), function ($form) use ($attributes) {
                $form->add(
                    'property_id',
                    SelectField::class,
            ...
            ->add(
                'projects_search_enabled',
                OnOffField::class,
                OnOffFieldOption::make()
                    ->label(__('Enable search projects on search box'))
                    ->defaultValue(true)
            )
            ->add(
                'default_search_type',
