...
$(() => {
    ...
    const rangeSlider = () => {
        ...
        const priceSlider = () => {
            ...
            $('[data-bb-toggle="range"]').each((index, el) => {
                ...
                noUiSlider.create(rangeSlider, {
                    start: [parseInt($minInput.val()) || 0, parseInt($maxInput.val()) || 0],
                    step: 1,
                    range: {
        ...
        const squareSlider = () => {
            ...
            noUiSlider.create(rangeSlider, {
                start: [parseInt($minSquare.val()), parseInt($maxSquare.val())],
                step: 1,
                range: {
        ...
        const flatSlider = () => {
            ...
            noUiSlider.create(rangeSlider, {
                start: [parseInt($minFlat.val()), parseInt($maxFlat.val())],
                step: 1,
                range: {
                    min: [$(rangeSlider).data('min')],
                    max: [$(rangeSlider).data('max')],
    ...
    const initProperties = () => {
        if ($('.tf-sw-property').length > 0) {
            const swiper = new Swiper('.tf-sw-property', {
                rtl: Theme.isRtl(),
                slidesPerView: 1,
                spaceBetween: 30,
                navigation: {
    ...
    $(document)
        ...
        .on('submit', '.filter-form', (e) => {
            ...
            const queryString = formData
                .filter((item) => item.name !== '_token' && item.value !== '')
                .map((item) => `${encodeURIComponent(item.name)}=${encodeURIComponent(item.value)}`)

            const nextHref = $form.prop('action') + (queryString.length > 0 ? `?${queryString.join('&')}` : '')
            ...
            $.ajax({
                ...
                success: function ({ error, data, message }) {
                    ...
                    if (nextHref !== window.location.href) {
                        window.history.pushState(formData, message, nextHref)
